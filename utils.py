...
def pad_sents_char(sents, char_pad_token):
    ...
    full_pad = [char_pad_token] * max_word_length
    max_sent_len = max(len(s) for s in sents)
    sents_padded = []
    for s in sents:
        sent_padded = []
        for w in s:
            if len(w) < max_word_length: # Most common case
                sent_padded.append(w + [char_pad_token] * (max_word_length-len(w))) # pad word
            elif len(w) > max_word_length:
                sent_padded.append(w[:max_word_length]) # truncate
            else:
                sent_padded.append(w)  # nothing else to do
        sents_padded.append(sent_padded + [full_pad] * (max_sent_len-len(sent_padded))) # pad sentence
    ### END YOUR CODE
