...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        for sent in sents:
            lst_chars=[]
            for wrd in sent:
                indx_lst = [self.char2id[char] for char in wrd]
                lst_chars.append([self.start_of_word]+indx_lst+[self.end_of_word])
            lst_wrds.append(lst_chars)
        return (lst_wrds)
        ### END YOUR CODE
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        ###     previous parts
        chr_indxs = self.words2charindices(sents)
        chr_indxs_p = pad_sents_char(chr_indxs,self.char2id['<pad>'])
